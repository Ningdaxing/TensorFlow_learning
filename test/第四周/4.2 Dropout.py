...
W2 = tf.Variable(tf.truncated_normal([500,1000],stddev=0.1))
b2 = tf.Variable(tf.ones([1000])+0.1)
L2 = tf.nn.tanh(tf.matmul(L1_drop,W2)+b2)
L2_drop = tf.nn.dropout(L2,keep_prob)

W3 = tf.Variable(tf.truncated_normal([1000,500],stddev=0.1))
b3 = tf.Variable(tf.ones([500])+0.1)
L3 = tf.nn.tanh(tf.matmul(L2_drop,W3)+b3)
L3_drop = tf.nn.dropout(L3,keep_prob)

W4 = tf.Variable(tf.truncated_normal([500,10],stddev=0.1))
b4 = tf.Variable(tf.ones([10])+0.1)
...
with tf.Session() as sess:
    ...
    for epoch in range(50):
        ...
        train_acc = sess.run(accuracy,feed_dict={x:mnist.train.images,y:mnist.train.labels,keep_prob:1.0})  #将训练好的模型，喂入测试数据
        print("Iter" + str(epoch) + ",Testing Accuracy " + str(test_acc) + ",Train Acc " + str(train_acc))